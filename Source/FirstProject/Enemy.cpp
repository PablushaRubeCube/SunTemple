...
void AEnemy::MoveToTarget(class AMainCharacter* Target) 
{
	...
	if (AIController)
	{
		...
		MoveRequest.SetAcceptanceRadius(25.f);// –ассто€ние с которого засчитаетс€ столкноение коллизий
		...
		/*ѕолучаем доступ к GetpathPoint, чтобы отслеживать начальную и конечную
		точку пути. ѕлюс дебаг сфера чтобы видеть в редакторе этот путь.
