...
AMainCharacter::AMainCharacter()
{
	...
	CameraBoom->bUsePawnControlRotation = true; // Rotate arm based on contorller

	//Create Follow Camera
	FollowCamera = CreateDefaultSubobject<UCameraComponent>(TEXT("FollowCamera"));
	FollowCamera->SetupAttachment(CameraBoom, USpringArmComponent::SocketName);
	/*Attach the camera to the end of the boom and let the boom
	the controoler oriention*/
	FollowCamera->bUsePawnControlRotation = false; 

	//Set size for collision capsule
	GetCapsuleComponent()->SetCapsuleSize(48.f, 108.f);
...
void AMainCharacter::Tick(float DeltaTime)
{
	...
	case EStaminaStatus::ESS_Normal:
		if (bShiftKeyDown && (bMoveRight || bMoveForward))
		{
...
void AMainCharacter::UpdateCombatTarget()
{
	...
	if (ClosestEnemy)
	{
		...
		float MinDistance = (ClosestEnemy->GetActorLocation() - CharLocation).Size();
		...
		for (auto Actor : OverlappingActors)
		{
			...
			if (Enemy)
			{
				float DistanceToActor = (Enemy->GetActorLocation() - CharLocation).Size();
